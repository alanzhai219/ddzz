...
// -----------------------------------------------------------------
// ---- 3. AVX-512 Intrinsics 实现 - 不展开 (OPT) ----
// -----------------------------------------------------------------
void decompress_avx512_nounroll_opt(uint8_t* decomp_buf,
    ...
    for (int block = 0; block < blocks; ++block) {
        ...
        // --- 缓存行对齐逻辑：按压缩流偏移对齐 ---
        size_t offset = (size_t)(current_src_ptr - compressed_buf);
...
// -----------------------------------------------------------------
// ---- 4. AVX-512 Intrinsics 实现 - 4x 展开 (OPT) ----
// -----------------------------------------------------------------
void decompress_avx512_opt(uint8_t* decomp_buf,
                           const uint8_t* compressed_buf,
                           const uint64_t* bitmask_ptr,
                           int blocks) {
    const uint8_t* current_src_ptr = compressed_buf;
