...
- `compare_blend/`: 用 SSE 按 lane 比较并选择 `max(a, b)`
- `movemask/`: 用 SIMD 比较生成 bitmask，再配合 `ctz` 找到首个命中位置
- `compress_store/`: 用 `movemask + shuffle` 把命中元素压缩写回连续输出
